...
def index(name):
    ...
    return exact,suffix
data={n:index(n) for n in ['ai-services','model-downloads','overseas-services','china-direct-v2']}
def has(n,d):
    exact,suffix=data[n];p=d.split('.')
    return d in exact or any('.'.join(p[i:]) in suffix for i in range(len(p)))
def route(d):
    for n,p in [('model-downloads','download'),('ai-services','AI'),('overseas-services','proxy'),('china-direct-v2','DIRECT')]:
        if has(n,d):return p
    return 'proxy'
def dns(d):
    if has('ai-services',d) or has('overseas-services',d):return 'overseas'
